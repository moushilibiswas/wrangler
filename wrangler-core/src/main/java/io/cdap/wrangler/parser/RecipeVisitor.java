...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  public RecipeSymbol.Builder visitByteSizeArg(DirectivesParser.ByteSizeArgContext ctx) {
    // Extract byte size string
    String byteSizeString = ctx.getText();
    
    // Create a ByteSize token
    builder.addToken(new ByteSize(byteSizeString));
  ...
  public RecipeSymbol.Builder visitTimeDurationArg(DirectivesParser.TimeDurationArgContext ctx) {
      // Extract time duration string
      String timeDurationString = ctx.getText();
      
      // Create a TimeDuration token
      builder.addToken(new TimeDuration(timeDurationString));
